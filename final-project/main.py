...
if __name__ == "__main__":
    ...
    min_lr_actor, max_lr_actor = 1e-7, 1e-2
    min_lr_critic, max_lr_critic = 1e-7, 1e-2
    learn_step = 100
    ...
    NET_CONFIG = {
        ...
        "head_config": {
            "hidden_size": [256, 256],  # Critic hidden size
        },
    }

    # Define the initial hy perparameters
    INIT_HP = {
        "POPULATION_SIZE": 10,
        "ALGO": "MATD3",  # Algorithm
        "BATCH_SIZE": 1024,  # Batch size
        "O_U_NOISE": True,  # Ornstein Uhlenbeck action noise
        "EXPL_NOISE": 0.04,  # Action noise scale
        "MEAN_NOISE": 0.0,  # Mean action noise
        "THETA": 0.15,  # Rate of mean reversion in OU noise
        "DT": 0.03,  # Timestep for OU noise
        "LR_ACTOR": lr_actor,  # Actor learning rate
        "LR_CRITIC": lr_critic,  # Critic learning rate
    ...
    hp_config = HyperparameterConfig(
        tau=RLParameter(min=1e-4, max=1e-2),
        policy_freq=RLParameter(min=1, max=5, dtype=int),
        lr_actor=RLParameter(min=min_lr_actor, max=max_lr_actor),
        lr_critic=RLParameter(min=min_lr_critic, max=max_lr_critic),
        batch_size=RLParameter(min=8, max=512, dtype=int),
        learn_step=RLParameter(
            min=20, max=learn_step, dtype=int, grow_factor=1.5, shrink_factor=0.75
        )
    )
    ...
    mutations = Mutations(
        no_mutation=0.99,  # Probability of no mutation
        ...
        rl_hp=0,  # Probability of RL hyperparameter mutation
        mutation_sd=0.01,  # Mutation strength
        rand_seed=1,
        device=device,
    )

    # Define training loop parameters
    max_steps = 7_000_000  # Max steps (default: 2000000)
    learning_delay = 0  # Steps before starting learning
    evo_steps = 60_000  # Evolution frequency
    ...
    # === Gráfico dos scores do AGENTE ELITE ===
    plt.figure(figsize=(12, 6))
    plt.plot(elite.scores, linewidth=2)
    plt.title('Evolução dos Scores do Agente Elite', fontsize=14)
    plt.xlabel('Episódios Completos', fontsize=12)
    plt.ylabel('Score do Episódio', fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    # Salvar o gráfico
    elite_scores_plot_path = os.path.join(path, "elite_scores_evolution.png")
    plt.savefig(elite_scores_plot_path, dpi=300, bbox_inches='tight')
    print(f"Gráfico da evolução da elite salvo em: {elite_scores_plot_path}")
